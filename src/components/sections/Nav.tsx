...
export default function Nav() {
  ...
  // Read the live data-theme value (set by the no-flash inline script in
  // layout.tsx). We never need to seed it ourselves — that was the bug that
  // forced dark on every reload regardless of OS preference.
  ...
  const toggleTheme = () => {
    ...
    try {
      ...
      // Ignore storage errors (private mode, quota, etc.) — theme still
      // updates for the current session via the data-theme attribute.
    }
  };
    ...
    <nav className="fixed inset-0 pointer-events-none z-50">
      <div className="absolute top-4 left-4 md:top-6 md:left-6 pointer-events-auto hidden sm:block">
        <div className="rounded-full border border-white/15 bg-black/45 backdrop-blur-md px-3.5 py-2 flex items-center gap-2.5">
          ...
          />
          <span className={`font-mono text-[10px] tracking-[0.32em] ${isLight ? "text-black/55" : "text-white/45"}`}>
            {indexLabel}
          </span>
          <span
            key={activeTracked.id}
            className={`font-mono text-[11px] tracking-[0.28em] nav-active-label ${isLight ? "text-black" : "text-white"}`}
          >
            {activeTracked.label}
          </span>
        </div>
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 pointer-events-auto">
        <div className={`rounded-full border border-white/20 bg-black/45 backdrop-blur-md px-3 sm:px-4 py-1.5 sm:py-2 font-mono text-[11px] sm:text-xs tracking-wide flex items-center gap-2 sm:gap-2.5 ${isLight ? "text-black/90" : "text-white/90"}`}>
          <span
            className="inline-block h-1.5 w-1.5 rounded-full"
            style={{ backgroundColor: ACCENT, boxShadow: `0 0 8px ${ACCENT}` }}
          />
          <span className={`tracking-[0.2em] ${isLight ? "text-black/65" : "text-white/55"}`}>MUMBAI</span>
          <span className={`h-3 w-px ${isLight ? "bg-black/25" : "bg-white/25"}`} />
          <span>{time} IST</span>
        </div>
      </div>

      <aside className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 hidden sm:flex items-center gap-4 pointer-events-auto">
        <div className={`h-[52vh] w-px relative ${isLight ? "bg-black/25" : "bg-white/30"}`}>
          <div
          ...
          {sectionLinks.map((section, index) => {
            ...
            return (
              ...
              >
                <span
                  className={`pointer-events-none absolute left-5 top-1/2 -translate-y-1/2 rounded px-2 py-1 font-mono text-[10px] tracking-wide opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap ${
                    isLight
                      ? "border border-black/25 bg-white/90 text-black"
                      : "border border-white/25 bg-black/70 text-white"
                  }`}
                >
                  {section.label}
                </span>
      ...
      <aside className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 hidden sm:flex flex-col items-center h-[50vh] justify-between pointer-events-auto">
        <div className={`w-px flex-1 ${isLight ? "bg-black/25" : "bg-white/30"}`} />
        <div className="flex flex-col items-center gap-4 py-4">
          ...
          >
            ...
            <span
              className={`pointer-events-none absolute right-6 top-1/2 -translate-y-1/2 rounded px-2 py-1 font-mono text-[10px] tracking-wide opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap ${
                isLight
                  ? "border border-black/25 bg-white/90 text-black"
                  : "border border-white/25 bg-black/70 text-white"
              }`}
          ...
          <a
            ...
            className={isLight ? "text-black/70 hover:text-black transition-colors" : "text-white/80 hover:text-white transition-colors"}
            aria-label="GitHub"
          >
